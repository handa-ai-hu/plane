...
1. 如果钉钉返回 `email`，使用真实邮箱。
2. 如果本地已有 `DingTalkUser(corp_id, union_id)`，使用已绑定的 Plane 用户。
3. 如果没有邮箱但有手机号，生成内部占位邮箱：

```text
dingtalk_<corp_id>_<union_id>@dingtalk.local
```

占位邮箱不对外发送邮件，只用于满足 Plane 用户模型约束。
